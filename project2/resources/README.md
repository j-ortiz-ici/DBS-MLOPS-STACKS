...
The databricks CLI bundles top level is defined by file `project2/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`project2/resources/model-workflow-resource.yml`)
 - batch inference workflow (`project2/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`project2/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`project2/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`project2/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `project2/databricks.yml` and `project2/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-project2-model     | /dev-project2-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-project2-model | /staging-project2-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-project2-model    | /prod-project2-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-project2-model    | /test-project2-experiment    |
...
2. Complete the `TODO` in `project2/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `project2/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `project2/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-project2-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-project2-batch-inference-job`
that runs the notebook under `project2/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-project2-batch-inference-job`.
...
`test-project2-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-project2-batch-inference-job`.
